#!/usr/bin/env python3
"""
Unified Simulator Controller WoT Operations Latency Test Script
Tests various WoT operations on simulator controllers (Gazebo or CoppeliaSim)
Measures response times and exports data to CSV

Usage:
    ./simulator_test.py gazebo        # Test Gazebo (gz_controller on port 8080)
    ./simulator_test.py coppeliasim   # Test CoppeliaSim (cs_controller on port 8080)
    ./simulator_test.py gz            # Short alias for gazebo
    ./simulator_test.py cs            # Short alias for coppeliasim

Configuration is automatically selected based on the simulator argument.
...
SIMULATOR_CONFIGS = {
    'gazebo': {
        ...
        'has_visualize': True,
        'ur10_cleanup': False,  # Skip UR10 cleanup for manual testing
    },
    'coppeliasim': {
        'name': 'CoppeliaSim',
...
# Add short aliases
SIMULATOR_CONFIGS['gz'] = SIMULATOR_CONFIGS['gazebo']
SIMULATOR_CONFIGS['cs'] = SIMULATOR_CONFIGS['coppeliasim']
...
TD_PATH = None
VISUALIZE_PATH = None
SIMCONTROL_PATH = None
MANAGEMODEL_PATH = None
MANAGESCENE_PATH = None
UR10_HOST = None
UR10_PORT = None
UR10_JOINTPOSITIONS_PATH = None
...
def parse_args():
    ...
    global CONFIG, HOST, PORT, SIMSTATS_PATH, TD_PATH, VISUALIZE_PATH
    global SIMCONTROL_PATH, MANAGEMODEL_PATH, MANAGESCENE_PATH
    global UR10_HOST, UR10_PORT, UR10_JOINTPOSITIONS_PATH
    
    parser = argparse.ArgumentParser(
        description='Unified WoT operations latency test for simulators',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s gazebo        # Test Gazebo controller
  %(prog)s coppeliasim   # Test CoppeliaSim controller
  %(prog)s gz            # Short alias for gazebo
  %(prog)s cs            # Short alias for coppeliasim
        """
    )
    parser.add_argument(
        'simulator',
        choices=['gazebo', 'coppeliasim', 'gz', 'cs'],
        help='Simulator to test'
    )
    
    args = parser.parse_args()
    
    # Load configuration for selected simulator
    CONFIG = SIMULATOR_CONFIGS[args.simulator]
    controller = CONFIG['controller']
    
    # Set global configuration variables
    HOST = CONFIG['host']
    PORT = CONFIG['port']
    SIMSTATS_PATH = f'/{controller}/properties/simStats'
    TD_PATH = f'/{controller}'
    VISUALIZE_PATH = f'/{controller}/properties/visualize'
    SIMCONTROL_PATH = f'/{controller}/actions/simControl'
    MANAGEMODEL_PATH = f'/{controller}/actions/manageModel'
    MANAGESCENE_PATH = f'/{controller}/actions/manageScene'
    UR10_HOST = CONFIG['host']
    UR10_PORT = CONFIG['ur10_port']
...
def run_setpose_test(path, name, iterations, model_file, model_name='test_setpose'):
    ...
    # Extract model ID if simulator uses ID-based identification (CoppeliaSim)
    model_id = None
    if CONFIG['uses_model_id']:
        model_id = spawn_result.get('data', {}).get('id')
        if model_id is None:
            print(f'✗ Failed to get model ID from spawn result')
            return results
        print(f'✓ {spawn_result["response_time"]}ms (id={model_id})')
    else:
    ...
    for i in range(1, iterations + 1):
        ...
        # Build setPose value - use id for CoppeliaSim, modelName for Gazebo
        setpose_value = {
            'mode': 'setPose',
        ...
        if CONFIG['uses_model_id']:
            setpose_value['id'] = model_id
        else:
            setpose_value['modelName'] = model_name
...
def run_spawn_delete_test(path, name, iterations, model_file, model_name_prefix='test_model', wait_after_spawn=2.0, wait_after_delete=None, position=None, orientation=None):
    ...
    for i in range(1, iterations + 1):
        ...
        if spawn_result['success']:
            # For CoppeliaSim, extract model ID (handle) from spawn response
            model_id = None
            if CONFIG['uses_model_id']:
                model_id = spawn_result.get('data', {}).get('id')
                if model_id is None:
                    error_msg = 'Failed to get model ID from spawn response'
        ...
        # Wait for entity to be fully registered and configured in simulator before deleting
        ...
        # Build delete value - use id for CoppeliaSim, modelName for Gazebo
        delete_value = {'mode': 'remove'}
        if CONFIG['uses_model_id'] and model_id is not None:
            delete_value['id'] = model_id
        else:
            delete_value['modelName'] = model_name
        ...
        if delete_result['success']:
            ...
            if 'ur10' in model_file.lower():
                ...
                for attempt in range(max_verify_attempts):
                    ...
                    models_path = f'/{CONFIG["controller"]}/properties/models'
                    check_result = wot_request(models_path, method='GET', timeout=5)
...
def run_ur10_jointpositions_test(iterations=ITERATIONS, model_name='ur10_joint_test', interval=1.0):
    """Load UR10, read jointPositions (with UR10 TD baseline), test moveToJoint, gripper, moveToCartesian"""
    print(f'\n{"="*80}')
    ...
    print(f'{CONFIG["name"]}: http://{HOST}:{PORT}{MANAGEMODEL_PATH}')
    ...
    # Step 3: Test moveToJoint multiple times
    print(f'\nTesting moveToJoint:')
    ...
    # Step 5: Test moveToCartesian multiple times
    ...
    # Step 6: Delete UR10 (if configured)
    if CONFIG['ur10_cleanup']:
        print(f'\n[Cleanup] Deleting {model_name}... ', end='', flush=True)
        delete_result = wot_request(
            MANAGEMODEL_PATH,
            method='POST',
...
def export_to_csv(all_results):
    ...
    csv_file = os.path.join(OUTPUT_DIR, f'{CONFIG["csv_prefix"]}_{timestamp}.csv')
...
def build_test_suite():
    """Build test suite based on simulator capabilities"""
    test_suite = [
        {
            'name': 'SimStats_Read',
            'enabled': True,
    ...
    # Add visualize test only for Gazebo
    if CONFIG['has_visualize']:
        test_suite.insert(1, {
...
def main():
    """Main execution"""
    # Parse arguments and configure for simulator
    args = parse_args()
    
    print('\n' + '█'*80)
    print(f'{CONFIG["name"].upper()} CONTROLLER WoT OPERATIONS LATENCY TEST')
    print('█'*80)
    print(f'\nSimulator: {CONFIG["name"]}')
    print(f'Controller: {CONFIG["controller"]} @ {HOST}:{PORT}')
    print(f'UR10 Server: {UR10_HOST}:{UR10_PORT}')
