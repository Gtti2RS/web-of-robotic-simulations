#!/usr/bin/env python3
"""
CoppeliaSim Controller WoT Operations Latency Test Script
Tests various WoT operations on cs_controller (port 8081)
Measures response times and exports data to CSV

Target: http://localhost:8081/cs_controller

Default All-Enabled Test Sequence (6 tests, ~125 operations):
┌─────────────────────────────────────────────────────────────────────────┐
...
│ 6. UR10_Full_Test         - Load UR10, jointPos 10x (UR10 TD baseline),│
│                             moveJoint 10x, gripper 5x, moveCart 10x    │
│                             (UR10 spawn ~23s, reads 1s, actions 2-5s)  │
└─────────────────────────────────────────────────────────────────────────┘

Baseline Normalization:
- TD baseline read before EACH SimStats and UR10 jointPositions operation
- cs_controller TD for SimStats, ur10_server TD for UR10 operations
...
Operation Breakdown:
- SimStats Reads: 10 ops + 10 TD baselines
- SimControl Run/Pause: 20 ops
- SimControl Speed: 20 ops
- Box Spawn-Delete: 20 ops (10 cycles)
- Box SetPose: 10 ops
...
CSV Export: cs_latency_YYYYMMDD_HHMMSS.csv
  Columns: ..., Baseline_Latency_ms, Response_Time_ms, Normalized_Latency_ms, ...

Note: UR10 full test included (same UR10 server works with both simulators)
...
SIMCONTROL_PATH = '/cs_controller/actions/simControl'
...
def run_setpose_test(path, name, iterations, model_file, model_name='test_setpose'):
    ...
    # Extract the model ID (handle) from spawn result
...
def run_spawn_delete_test(path, name, iterations, model_file, model_name_prefix='test_model', wait_after_spawn=2.0, wait_after_delete=None, position=None, orientation=None):
    ...
    for i in range(1, iterations + 1):
        ...
        if spawn_result['success']:
            # Extract model ID (handle) from spawn response for CoppeliaSim
            model_id = spawn_result.get('data', {}).get('id')
            if model_id is None:
        ...
        # Wait for entity to be fully registered and configured in CoppeliaSim before deleting
        time.sleep(wait_after_spawn)
        
        # Delete model using ID instead of modelName (CoppeliaSim requirement)
        ...
        if delete_result['success']:
            ...
            if 'ur10' in model_file.lower():
                ...
                for attempt in range(max_verify_attempts):
                    ...
                    check_result = wot_request('/cs_controller/properties/models', method='GET', timeout=5)
...
def run_ur10_jointpositions_test(iterations=ITERATIONS, model_name='ur10_joint_test', interval=1.0):
    """Load UR10, read jointPositions (with UR10 TD baseline), test moveToJoint, gripper, moveToCartesian, then remove"""
    print(f'\n{"="*80}')
    ...
    print(f'CoppeliaSim: http://{HOST}:{PORT}{MANAGEMODEL_PATH}')
...
def export_to_csv(all_results):
    ...
    csv_file = os.path.join(OUTPUT_DIR, f'cs_latency_{timestamp}.csv')
...
TEST_SUITE = [
    ...
    # },
    {
        ...
        'enabled': True,
        'description': 'Spawn-Delete ur10_rg2_coppelia.urdf 10 times',
        'function': lambda: run_spawn_delete_test(MANAGEMODEL_PATH, 'manageModel', ITERATIONS, 'ur10_rg2_coppelia.urdf', 'test_ur10', wait_after_spawn=5.0, wait_after_delete=5.0),
        'stats_title': 'Spawn-Delete UR10 Robot (ur10_rg2_coppelia.urdf)'
    },
]
...
def main():
    """Main execution"""
    print('\n' + '█'*80)
    print('COPPELIASIM CONTROLLER WoT OPERATIONS LATENCY TEST')
    print('█'*80)
